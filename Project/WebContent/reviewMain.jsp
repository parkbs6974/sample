...
<%
   ...
   List<ReviewVO> list = ReviewDAO.getReviewList(p.getBegin(), p.getEnd());
   ...
   ReviewVO sessionLvo = (ReviewVO)session.getAttribute("lvo"); 
...
main > aside > .navform > .category ol > li > a:hover {
	...
	<%@ include file="common/userHeader.jspf" %>
		...
		<form method="post" class="navform" name="testForm">
           ...
           <tr>
            <c:forEach var="vo" items="${list }">
               <div class="review-item">
               	<c:if test="${not empty vo.reviewFile }">
                  <img alt="사진" src="images/${vo.reviewFile}" class="review-img">
                </c:if>
                <c:if test="${empty vo.reviewFile }">
                   <img alt="사진" src="images/noImage.jpg" class="review-img">
                </c:if>
                    <div class="review-content">
                        <div class="review-author"><a href="reviewView.jsp?reviewNum=${vo.reviewNum}&cPage=${pvo.nowPage}">${vo.id }</a></div>
                        <div class="review-date">작성날짜: ${vo.reviewDate }</div>
                        <div class="review-text">제목: ${vo.reviewTitle }</div>
                        <div class="review-text">별점: ${vo.reviewStar }/5</div>
                        <div class="review-text">내용: ${vo.reviewCont }</div>
                    </div>
                </div>
                </c:forEach>
                </tr>
		    ...
		    <ol class="paging">
		      ...
		      <c:if test="${pvo.beginPage != 1 }">
		        <li class="paging-item">
		          <a class="paging-link" href="reviewMain.jsp?cPage=${pvo.beginPage -1 }">이전으로</a>
		        </li>
		      </c:if> 
		      ...
		      <c:forEach var="pageNo" begin="${pvo.beginPage }" end="${pvo.endPage }">
		        ...
		        <c:if test="${pageNo != pvo.nowPage}">
		          <li class="paging-item"><a class="paging-link" href="reviewMain.jsp?cPage=${pageNo }">${pageNo }</a></li>
		        </c:if>
		      </c:forEach>
		      ...
		      <c:if test="${pvo.endPage < pvo.totalPage }">
		        <li class="paging-item">
		          <a class="paging-link" href="reviewMain.jsp?cPage=${pvo.endPage + 1 }">다음으로</a>
		        </li>
		      </c:if>
		      <c:if test="${pvo.endPage >= pvo.totalPage }">
		  ...
		  <span class="review-write-button"></span>
		  </td>     
		</tfoot>   
		  </table>
		</section>
